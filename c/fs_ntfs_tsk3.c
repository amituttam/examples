...
static fs_ret_E
fs_ntfs_get_size( const char* device, uint64_t *sz )
{
	int fd;
	...
	if (fd == -1)
		printf("fs_ntfs_get_size: Could not open disk %s\n", device);
	else
	{
		if (ioctl(fd, BLKGETSIZE64, sz) != -1) 
		{
			printf("fs_ntfs_get_size: BLKGETSIZE64: %llu\n", *sz);
			ret = FS_SUCCESS;
		}
		else
			printf("fs_ntfs_get_size: Error in getting BLKGETSIZE64\n");
		close(fd);
	}
...
fs_ntfs_walk_cb(const TSK_FS_BLOCK *a_block, void *a_ptr)
{
...
static fs_ret_E
fs_ntfs_read_bitmap( NTFS_INFO *ntfs, fs_data_S *fsinfo, fs_bitmap_S **bm )
{
	fs_ret_E result = FS_SUCCESS;
	uint64_t current_block;
	uint64_t used_blocks = 0;
	unsigned int bitmap_size = ( ntfs->fs_info.block_count + 7 ) / 8;
	...
	ntfs->fs_info.block_walk(&ntfs->fs_info,
	                         ntfs->fs_info.first_block,
	                         ntfs->fs_info.last_block_act,
...
#if 0
	for ( current_block = 0; current_block < ntfs->fs_info.block_count; current_block++ )
	{
		TSK_FS_BLOCK_FLAG_ENUM bit = ntfs->fs_info.block_getflags(&ntfs->fs_info, current_block);
...
#endif
	fsinfo->blk_used = fsinfo->blk_count - fsinfo->blk_free;
...
fs_ntfs_identify( const char *src, fs_data_S *inout, fs_bitmap_S **bm )
{
	...
	/* Size from kernel */
	if (fs_ntfs_get_size(src, &dev_sz) == FS_SUCCESS)
	{
	...
	inout->blk_size = ntfs->csize_b;
	inout->blk_count = ntfs->fs_info.block_count;

	/* Size */
	inout->fs_size = dev_sz;

	/* Bitmap */
	result = fs_ntfs_read_bitmap( ntfs, inout, bm );
