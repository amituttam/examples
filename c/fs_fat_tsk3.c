...
static fs_ret_E
fs_fat_get_size( const char* device, uint64_t *sz )
{
	int fd;
	...
	if (fd == -1)
		printf("fs_fat_get_size: Could not open disk %s\n", device);
	else
	{
		if (ioctl(fd, BLKGETSIZE64, sz) != -1) 
		{
			printf("fs_fat_get_size: BLKGETSIZE64: %llu\n", *sz);
			ret = FS_SUCCESS;
		}
		else
			printf("fs_fat_get_size: Error in getting BLKGETSIZE64\n");
		close(fd);
	}
...
fs_fat_walk_cb(const TSK_FS_BLOCK *a_block, void *a_ptr)
{
...
static fs_ret_E
fs_fat_read_bitmap( FATFS_INFO *fat, fs_data_S *fsinfo, fs_bitmap_S **bm )
{
	fs_ret_E result = FS_SUCCESS;
	uint64_t current_block;
	uint64_t used_blocks = 0;
	unsigned int bitmap_size = ( fat->fs_info.block_count + 7 ) / 8;
...
#if 0
	fat->fs_info.block_walk(&fat->fs_info,
	                         fat->fs_info.first_block,
	                         fat->fs_info.last_block_act,
	...
	for ( current_block = 0; current_block < fat->fs_info.block_count; current_block++ )
	{
		TSK_FS_BLOCK_FLAG_ENUM bit = fat->fs_info.block_getflags(&fat->fs_info, current_block);
	...
	printf("fs_fat_read_bitmap: free: %llu used: %llu\n",
...
fs_fat_identify( const char *src, fs_data_S *inout, fs_bitmap_S **bm )
{
	...
	TSK_FS_INFO *fs;
	TSK_IMG_INFO *img;
	TSK_FS_TYPE_ENUM fstype = TSK_FS_TYPE_FAT_DETECT;
	TSK_IMG_TYPE_ENUM imgtype = TSK_IMG_TYPE_DETECT;

	/* Open the FAT filesystem. This is opened as an image first and
	...
	/* Size from kernel */
	if (fs_fat_get_size(src, &dev_sz) == FS_SUCCESS)
	{
		...
		return FS_GENERAL_ERROR;

	/* FAT */
	FATFS_INFO *fat = (FATFS_INFO *) fs;

	/* FAT Type */
	inout->type = FS_TYPE_FAT;
	if (fat->fs_info.ftype == TSK_FS_TYPE_FAT32)
		strcpy(inout->type_str, "FAT32");
	else if (fat->fs_info.ftype == TSK_FS_TYPE_FAT16)
		strcpy(inout->type_str, "FAT16");
	else
		strcpy(inout->type_str, "FAT12");

	/* Size */
	inout->blk_size = fat->ssize;
	inout->blk_count = fat->fs_info.block_count;
	inout->fs_size = dev_sz;

	/* Bitmap */
	result = fs_fat_read_bitmap( fat, inout, bm );
